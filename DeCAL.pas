...
{$ENDIF}
  ...
  PDIterator = ^DIterator;
	DIterator = record
		...
		Handler : Pointer; // JSB: This used to be a IIterHandler, but to avoid interface calls to _Release with destroyed pointer we will case whenever necessary
  ...
  ISequence = interface (IContainer)
    ...
    procedure replaceWithin(_begin, _end : Integer; sources, targets : array of 
        const);
    procedure _add(const obj : DObject);
  ...
  IVector = interface (ISequence)
    ...
    procedure insertAtIter(iterator : DIterator; objs : array of const);
    procedure insertMultipleAt(index : Integer; count : Integer; obj : array of 
        const);
    procedure insertMultipleAtIter(iterator : DIterator; count : Integer; obj :
        array of const);
    ...
    procedure _insertMultipleAt(index : Integer; count : Integer; const obj : 
        DObject);
    procedure _insertMultipleAtIter(iterator : DIterator; count : Integer; const 
        obj : DObject);
    procedure _removeWithin(_begin, _end : Integer; const obj : DObject);
  end;
...
{$ENDIF}
  ...
  public
    ...
    procedure RegisterContainerClass(const AContainerName : string; AContainerClass
        : DContainerClass);
    procedure UnRegisterContainerClass(const AContainerName : string);
    function CreateContainerWith(const AContainerName : string; compare : 
        Dcomparator): DContainer;
  end;
...
{$ENDIF}
  ...
  function copyTStringsIntoAssociative(Source : TStrings; Dest : IAssociative):
      DIterator;

  { Copies a IAssociative into a TStrings }

  function copyAssociativeIntoTStrings(Source : IAssociative; Dest : TStrings):
      DIterator;
...
{function _removeCopy(source, destination : IContainer; const obj : DObject) : DIterator;
 ...
 function CopyRemoveFromTarget(source, destination : IContainer;  ACopyKey : 
     boolean; objs : array of const): DIterator;
 ...
 function CopyRemoveToFromTarget(source : IContainer; dest : DIterator; ACopyKey 
     : boolean; objs : array of const): DIterator;
 ...
 function CopyRemoveInFromTarget(_start, _end, dest : DIterator; ACopyKey : 
     boolean; objs : array of const): DIterator;
 ...
 function CopyRemoveIfFromTarget(source, destination : IContainer; test : DTest; 
 ...
 function CopyRemoveIfToFromTarget(source : IContainer; dest : DIterator; test : 
     DTest; ACopyKey : boolean): DIterator;
 ...
 function CopyRemoveIfInFromTarget(_start, _end, dest : DIterator; test : DTest; 
     ACopyKey : boolean): DIterator;
...
function getObject(const iterator : DIterator) : TObject;
begin
	assert((IIterHandler (iterator.Handler).iget(iterator).VType = vtObject) or (IIterHandler (iterator.Handler).iget(iterator).VInteger = 0));
...
begin
  ...
  assert(obj1.VType = obj2.Vtype);
	...
	case obj1.VType of
      vtInteger:
      	result := SignInteger(obj1.VInteger, obj2.VInteger);
      vtBoolean:
      ...
      vtPointer:
      	result := SignInteger(Integer(obj1.VPointer), Integer(obj2.VPointer));
      vtPChar:
      	result := SignInteger(Integer(obj1.VPChar), Integer(obj2.VPChar));
      vtObject:
      	result := SignInteger(Integer(obj1.VObject), Integer(obj2.VObject));
      vtClass:
      	result := SignInteger(Integer(obj1.VClass), Integer(obj2.VClass));
      vtWideChar:
      	result := Ord(obj1.VWideChar) - Ord(obj2.VWideChar);
      vtPWideChar:
      	result := SignInteger(Integer(obj1.VPWideChar), Integer(obj2.VPWideChar));
      vtAnsiString:
      ...
      vtInterface:
      	result := SignInteger(Integer(obj1.VInterface), Integer(obj2.VInterface));
      vtWideString:
       result := SignWideString(WideString(obj1.VWideString), WideString(obj2.VWideString));
  end;
...
begin
  ...
  while not atEnd(iter) do
      ...
      Obj := getObject(iter);
      if (Obj <> nil) and Obj.GetInterface (IContainer, con)
        {con <> nil}
        then
      	begin
	      	con._clear(direct);
...
{$IFDEF FREEPOSSIBLE}
	while not equals(_start, _end) do
		begin
     if getRef(_start)^.VObject <> nil
       then getRef(_start)^.VObject.Free;
			advance(_start);
		end;
{$ELSE}
...
begin
    ...
    if not found then
     begin
       if ACopyKey
         then SetToKey (_start)
         else SetToValue (_start);
       try
         oKey := getRef(_start);
       finally
         // Sets contain a null value in the second half of the
...
begin
  for i := 0 to Source.Count - 1 do
    begin
      if SplitNameAndValue (Source [i], AName, AValue)
        then Dest.putPair ([AName, AValue]);
    end;
  Result := Dest.finish;
end;
...
begin
  ...
  if not atEnd (iter)
    then Result := DContainerClass (GetClass (Iter)).Create
    else Result := nil;
end;

procedure DFactory.RegisterContainerClass(const AContainerName : string;
    AContainerClass : DContainerClass);
var
  Iter : DIterator;
begin
  Iter := FContainerClasses.locate ([AContainerName]);
  if not atEnd (Iter)
    then (FContainerClasses as DAssociative).removeAt (Iter);
...
function DFactory.CreateContainerWith(const AContainerName : string; compare :
    Dcomparator): DContainer;
var
  Iter : DIterator;
begin
  Iter := FContainerClasses.locate ([AContainerName]);
  if not atEnd (iter)
    then Result := DContainerClass (GetClass (Iter)).createWith (compare)
    else Result := nil;
end;
